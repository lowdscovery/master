     ...
     @if ($isSelected)
        <form wire:submit.prevent="AddDistrict">
            <div class="p-4">
                    ...
                    <div class="form-row">
                    <div class="col">
     <input type="text" class="form-control @error('nom') is-invalid @enderror" wire:model="nom" placeholder="District" required="required" title="District">
                   @error("nom")
                       ...
                       @forelse ($districts as $district)
                         <tr>
                            ...
                            <td class="text-center">
                                ...
                                @can('delete', $district)
                                <button class="btn btn-link" wire:click="confirmDelete({{$district->id}})" style="color:red;"> <i class="far fa-trash-alt"></i> </button>
                                @endcan
                            </td>
                            @endcan
                        </tr>
       ...
       Swal.fire({
        ...
        if (result.isConfirmed) {
            if(event.detail.message.data){
               @this.deleteDistrict(event.detail.message.data.district_id)
            }
        //    @this.resetPassword()
        }
        })
